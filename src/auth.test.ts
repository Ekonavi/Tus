...
describe("Auth", async () => {
  ...
  it("rejects expired credentials", async () => {
    expect(await validateAt(user, await generatePassAt(user, 1), 12)).toBe(
      false,
    );
  });

  it("passes valid credentials", async () => {
    expect(await validateAt(user, await generatePassAt(user, 1), 11)).toBe(
      true,
    );
  });

  it("rejects wrong-user credentials", async () => {
    expect(await validateAt(user, await generatePassAt(user + "a", 1), 1)).toBe(
      false,
    );
  });
