...
describe("uploadHandler", () => {
  ...
  it("cleans after alarms", async () => {
    ...
    await runInDurableObject(stub, async (_instance, state) => {
      expect(await state.storage.get("upload-info")).toMatchObject({
        uploadLength: 10,
      });
  ...
  it("cleans after unrecoverable failure", async () => {
    ...
    await expect(() =>
      stub.fetch("http://localhost/upload/bucket/test123", {
        method: "PATCH",
  ...
  it("hydrates tx parts from cold storage", async () => {
    ...
    expect(read?.slice(partBody.length, partBody.length + 10)).toBe(
      "123456789!",
    );
  ...
  test.each([
    ...
    "resumes from storage for chunks=[%s,%s,%s]",
    async (chunk1Size, chunk2Size, chunk3Size) => {
      ...
      const totalLength =
        firstChunk.length + secondChunk.length + thirdChunk.length;

      const id = handler.idFromName("test123");
      let stub = handler.get(id);
      await stub.fetch(
        new Request("http://localhost/upload/bucket", {
          method: "POST",
          headers: {
            "Upload-Metadata": `filename ${btoa("test123")}`,
      ...
      await stub.fetch(
        new Request("http://localhost/upload/bucket/test123", {
          method: "PATCH",
