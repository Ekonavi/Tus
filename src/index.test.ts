...
describe("worker auth", () => {
  it("rejects un-authd request", async () => {
    const res = await SELF.fetch(
      `http://localhost/upload/${attachmentsPath}/`,
      {
        method: "POST",
  ...
  it.each(["write", "", "abc"])(
    "rejects GET to backups with %s permission",
    async (permission) => {
      const res = await SELF.fetch(`http://localhost/${backupsPath}/abc/def`, {
  ...
  it.each(["ab", "/abc", "abc/", "", "abc/def"])(
    "rejects GET with incorrect subdir %s",
    async (subdir) => {
      const res = await SELF.fetch(`http://localhost/${backupsPath}/abc/def`, {
...
describe("request validation", () => {
  ...
  it("accepts no trailing slash", async () => {
    ...
    expect(res.headers.get("location")).toBe(
      `http://localhost/upload/${attachmentsPath}/abc`,
    );
    await res.body?.cancel();
  });
});
...
describe("Tus", () => {
  ...
  async function patchRequest(
    ...
    return await SELF.fetch(
      `http://localhost/upload/${attachmentsPath}/${name}`,
      {
        method: "PATCH",
        headers: h,
        body: body,
  ...
  it("handles ranged reads", async () => {
    ...
    expect(prefixResponse.headers.get("content-range")).toBe(
      `bytes 0-371/${body.length}`,
    );
    ...
    expect(suffixResponse.headers.get("content-range")).toBe(
      `bytes 372-${body.length - 1}/${body.length}`,
    );
    expect(suffix).toEqual(body.slice(372));
  });

  test.each(["nibbles=0-3", "bytes=0-2,4-", "bytes=zzz"])(
    "ignores bad range: %s",
    async (arg: string) => {
  ...
  it("handles suffix ranges", async () => {
    ...
    expect(response.headers.get("content-range")).toEqual(
      `bytes ${body.length - 99}-${body.length - 1}/${body.length}`,
    );
  });

  test.each([1, 2, 17])(
    "handles reading chunks of length=%s",
    { timeout: 60000 },
    async (chunkSize: number) => {
      ...
      for (let offset = 0; offset < body.length; offset += chunkSize) {
        ...
        const response = await getRequest({
          range: `bytes=${offset}-${endIndex}`,
        });
        expect(response.status).toBe(206);
        expect(response.headers.get("content-range")).toBe(
          `bytes ${offset}-${endIndex}/${body.length}`,
        );
  ...
  test.each([0, 1, PART_SIZE - 1, PART_SIZE, PART_SIZE + 1])(
    "rejects incorrect checksum for length=%s",
    async (bodySize: number) => {
      await createRequest({
        uploadLength: bodySize,
      ...
      const head = await headRequest();
      if (head.status === 200) {
        expect(head.headers.get("Upload-Offset")).toBe(
          head.headers.get("Upload-Length"),
        );
      } else {
  ...
  test.each([
    ...
    async (bodySize: number, multiplePatches: boolean) => {
      const expectedChecksum = await sha256(
        body(bodySize, { pattern: "test" }),
      );
      await createRequest({
        uploadLength: bodySize,
        checksum: new Uint8Array(expectedChecksum),
      });
      if (multiplePatches) {
      ...
      const actualChecksum = Buffer.from(
        get.headers.get(X_SIGNAL_CHECKSUM_SHA256) || "",
        "base64",
      );
  ...
  test.each([
    ...
    PART_SIZE + 1,
    PART_SIZE * 10 + 1,
  ])("upload(%s bytes)", { timeout: 60000 }, async (uploadSize) => {
...
describe("completed object read operations", () => {
  test.each(["HEAD", "GET"])(
    "404s unknown %s object",
    async (method: string) => {
      const head = await SELF.fetch(
        `http://localhost/${backupsPath}/subdir/does_not_exist`,
        {
          method,
          headers: { Authorization: await backupHeaderFor("subdir", "read") },
        },
  ...
  test.each(["HEAD", "GET"])(
    "populates headers for %s object",
    async (method: string) => {
      ...
      const resp = await SELF.fetch(
        `http://localhost/${backupsPath}/subdir/a/b`,
        {
          method: method,
          headers: { Authorization: await backupHeaderFor("subdir", "read") },
        },
      ...
      expect(resp.headers.get("etag")).toBe(
        await s3Etag(body(4, { pattern: "test" })),
      );
      ...
      const diffSeconds =
        Math.abs(new Date().getTime() - lastModified.getTime()) / 1000;
...
function bodyMatchesPattern(body: string, pattern: string): boolean {
  for (let offset = 0; offset < body.length; offset += pattern.length) {
    ...
    if (
      body.slice(offset, offset + pattern.length) !==
      pattern.slice(0, remaining)
    ) {
      return false;
    }
  }
...
async function s3Etag(body: ReadableStream<Uint8Array>): Promise<string> {
  ...
  const nestedMd5 = await crypto.subtle.digest(
    "MD5",
    await new Blob(md5sums).arrayBuffer(),
  );
